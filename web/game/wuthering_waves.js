...
const getCanonicalWuwaFilename = (filename, json) => {
  if (!filename || !json || typeof json !== 'object') return filename;
  const match = filename.match(/^wuwa_(.+)\.json$/i);
  if (!match) return filename;
  const profile = json.profilename || json.profileName || json.profile_name;
  if (!profile) return filename;
  const canonical = `wuwa_${String(profile).trim()}.json`;
...
const getJsonSideRailItems = json => {
  ...
  if (Array.isArray(json.side_rail_items) && json.side_rail_items.length > 0) {
    return json.side_rail_items.map((item, index) => ({
      ...
      poolType: String(item.poolType || item.pool_type || item.pool || index + 1)
    }));
  }
...
const renderStats = rows => {
  ...
  const totalStars = totalPulls * 160;
